...
const Inject = <T = unknown>(serviceIdentifier: any) => (target: any, targetKey?: string | symbol | undefined, indexOrPropertyDescriptor?: number | TypedPropertyDescriptor<any> | undefined) => {
    inject(serviceIdentifier)(target, targetKey, indexOrPropertyDescriptor)
}

const InjectableHandler = (command: new (...args: never[]) => ICommand<any>) => {
